...
public class Polygon {
    ...
    public LineSegment[] sides() {
        if (sides == null) {
            sides = new LineSegment[vertices.length];
            for (int i = 0; i < vertices.length - 1; i++) {
                sides[i] = new LineSegment(vertices[i], vertices[i + 1]);
            }
            sides[sides.length - 1] = new LineSegment(vertices[vertices.length - 1], vertices[0]);
        }
        return sides;
    }
